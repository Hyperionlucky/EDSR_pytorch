...
class Trainer():
    def __init__(self, args, loader, my_model, my_loss):
        ...
        self.train_evaluator = Evaluator(self.args.batch_size)
        self.val_evaluator = Evaluator(self.args.test_batch_size)
    ...
    def train(self,epoch):
        ...
        self.writer.add_scalar("train/mse", mse.item(), epoch)
        self.writer.add_scalar("train/PSNR", psnr.item(), epoch)
        self.writer.add_scalar("train/MAE", mae.item(), epoch)
        self.writer.add_scalar("train/RMSE", rmse.item(), epoch)
        ...
        print("MSE:{}, PSNR:{}, MAE:{}, RMSE:{}".format(mse.item(), psnr.item(), mae.item(), rmse.item()))
        print('Loss: %.3f' % (train_loss/i))
        print("\nParams: %.1fM" %(params_num / 1e6))

        logging.info("train:")
        logging.info('[Epoch: %d, numImages: %5d]' % (epoch, self.num_trainImage))
        logging.info("MSE:{}, PSNR:{}, MAE:{}, RMSE:{}".format(mse.item(), psnr.item(), mae.item(), rmse.item()))
    ...
    def test(self, epoch):
        ...
        mse, psnr, mae, rmse = self.val_evaluator.score(self.num_valImage)
        self.writer.add_scalar("val/loss_epoch", val_loss/(i+1), epoch)
        self.writer.add_scalar("val/mse", mse.item(), epoch)
        self.writer.add_scalar("val/PSNR", psnr.item(), epoch)
        self.writer.add_scalar("val/MAE", mae.item(), epoch)
        self.writer.add_scalar("val/RMSE", rmse.item(), epoch)
