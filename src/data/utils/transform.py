...
class Totensor(object):
    def __call__(self, hr,lr,slope):
        ...
        hr = hr[:,:,np.newaxis].astype(np.int16) 
        hr = torch.from_numpy(hr)
        hr = hr.permute(2,0,1)

        lr = lr[:,:,np.newaxis].astype(np.int16) 
        lr = torch.from_numpy(lr)
        lr = lr.permute(2,0,1)

        slope = slope[:,:,np.newaxis].astype(np.int16)
        slope = torch.from_numpy(slope)
        slope = slope.permute(2,0,1)

        return hr.long(),lr.long(),slope.float()
...
class RandomScaleCrop(object):
    ...
    def __call__(self,hr,lr,slope):
        ...
        hr = hr[crop_h*2:crop_h*2+self.crop_size_h,crop_W*2:crop_W*2+self.crop_size_h]
        lr = lr[crop_h:crop_h+self.crop_size_l,crop_W:crop_W+self.crop_size_l]
        slope = slope[crop_h:crop_h+self.crop_size_l,crop_W:crop_W+self.crop_size_l]
...
class RandomRotation(object):
    ...
    def __call__(self,hr,lr,slope):
        ...
        hr = (transform.rotate(hr / 2000,self.rotation) * 2000).astype(np.int16)
        lr = (transform.rotate(lr / 2000,self.rotation) * 2000).astype(np.int16)
