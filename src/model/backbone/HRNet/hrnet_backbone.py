...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, bn_momentum=0.01):
...
class HighResolutionModule(nn.Module):
    def __init__(self, num_branches, blocks, num_blocks, num_inchannels,
                 num_channels, fuse_method, multi_scale_output=True,  bn_momentum=0.01):
        ...
        self.branches = self._make_branches(
            num_branches, blocks, num_blocks, num_channels,  bn_momentum=bn_momentum)
        self.fuse_layers = self._make_fuse_layers( bn_momentum=bn_momentum)
        self.relu = nn.ReLU(inplace=False)
    ...
    def _make_branches(self, num_branches, block, num_blocks, num_channels,  bn_momentum=0.01):
        branches = []

        for i in range(num_branches):
            branches.append(
                self._make_one_branch(i, block, num_blocks, num_channels,  bn_momentum=bn_momentum)
            )

        return nn.ModuleList(branches)

    def _make_fuse_layers(self, bn_momentum=0.01):
...
class HighResolutionNet(nn.Module):
    ...
    def __init__(self, cfg,  bn_momentum=0.01, **kwargs):
        self.inplanes = 64
        ...
        self.transition1 = self._make_transition_layer([stage1_out_channel], num_channels,  bn_momentum=bn_momentum)
        
        self.stage2, pre_stage_channels = self._make_stage(
            self.stage2_cfg, num_channels, bn_momentum=bn_momentum)
        ...
        self.transition2 = self._make_transition_layer(
            pre_stage_channels, num_channels,  bn_momentum=bn_momentum)
        self.stage3, pre_stage_channels = self._make_stage(
            self.stage3_cfg, num_channels,  bn_momentum=bn_momentum)
        ...
        self.stage4, pre_stage_channels = self._make_stage(
            self.stage4_cfg, num_channels, multi_scale_output=True,  bn_momentum=bn_momentum)

        self.last_inp_channels = np.int(np.sum(pre_stage_channels))
        self.channels = [self.last_inp_channels]
    ...
    def _make_head(self, pre_stage_channels,  bn_momentum):
    ...
    def _make_transition_layer(
            self, num_channels_pre_layer, num_channels_cur_layer,  bn_momentum):
    ...
    def _make_layer(self, block, inplanes, planes, blocks, stride=1, bn_momentum=0.01):
        downsample = None
        ...
        layers = []
        layers.append(block(inplanes, planes, stride, downsample, bn_momentum=bn_momentum))
    ...
    def _make_stage(self, layer_config, num_inchannels,
                    multi_scale_output=True, bn_momentum=0.01):
