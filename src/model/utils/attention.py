...
class CrossScaleAttention(nn.Module):
    ...
    def forward(self, input):
        #get embedding
        embed_w = self.conv_assembly(input) # 生成v
        match_input = self.conv_match_1(input) # 生成Q
        
        # b*c*h*w
        shape_input = list(embed_w.size())   # b*c*h*w
        input_groups = torch.split(match_input,1,dim=0) # 对Q按batch分离
