...
from model.utils import mpnconv
...
class SOCA(nn.Module):
    ...
    def forward(self, x):
        ...
        cov_mat_sqrt = mpnconv.SqrtmLayer(cov_mat,5) # Matrix square root layer( including pre-norm,Newton-Schulz iter. and post-com. with 5 iteration)
        ##
...
class SAN(nn.Module):
    def __init__(self, args) -> None:
        ...
        # define head module
        m_head = [conv(args.n_channels, n_feats, 3)]        #第一个卷积层

        # define body module
        self.RG = nn.ModuleList([LSRAG(conv, n_feats, 3, 16, \
                                              act=act, res_scale=args.res_scale, n_resblocks=n_resblocks) for _ in range(20)])
        self.conv_last = conv(n_feats, n_feats, 3)

        self.non_local = Nonlocal_CA(in_feat=n_feats, inter_feat=n_feats//8, reduction=8,sub_sample=False, bn_layer=False)
        # define tail module
        m_tail = [                                                       #上采样
            common.Upsampler(conv, scale, n_feats, act=False),
            conv(n_feats, args.n_channels, 3)
        ]
